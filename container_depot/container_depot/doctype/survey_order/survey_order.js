...
frappe.ui.form.on("Survey Order", {
	setup(frm) {
		...
		frm.set_query("item", "charges", () => ({
			query: "container_depot.container_depot.doctype.survey_order.survey_order.item_price_query",
	...
	paid_to(frm) {
		...
		frappe.call({
			method: "container_depot.container_depot.doctype.survey_order.survey_order.get_pricing_context",
...
frappe.ui.form.on("Survey Order Charge", {
	...
	item(frm, cdt, cdn) {
		...
		frappe.call({
			method: "container_depot.container_depot.doctype.survey_order.survey_order.get_item_price",
