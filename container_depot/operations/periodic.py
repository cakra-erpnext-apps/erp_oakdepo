...
Mirrors ``operations/mr.py``: an owner-approval status machine (Draft → Service Setup →
...
# stay byte-for-byte consistent (operations/mr.py is the origin).
from container_depot.operations.mr import (
	_CONTAINER_FIELDS,
	_as_bool,
	_clean,
	_on_hand,
...
# Owner-approval status machine — identical shape to M&R (operations/mr.MR_TRANSITIONS).
...
def issue_parts_stock(order):
	...
	Mirrors ``operations/mr._issue_parts_stock`` (per-row warehouse). Most periodic lines are
...
# --- PWA execution console (mirrors operations/mr.py) ------------------------
# The functions below back the ESS PWA wrappers (ess/periodic.py). Kept free of
# @frappe.whitelist like operations/mr.py — the endpoint layer adds auth + whitelisting.
