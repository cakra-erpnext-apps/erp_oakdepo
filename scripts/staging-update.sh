...
for _ in $(seq 1 60); do
  if docker exec erp_oakdepo_staging-frontend-1 wget -q -O /dev/null --header "Host: $SITE" http://backend:8000/api/method/ping; then
    up=1
    break
  fi
  sleep 2
...
if [ "$up" != "1" ]; then
  ...
  dc ps
  docker logs --tail 80 erp_oakdepo_staging-backend-1 || true
  exit 1
fi
